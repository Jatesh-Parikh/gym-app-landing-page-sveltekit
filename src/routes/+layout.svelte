...
{#if $openModal}
  <div class="fixed top-0 left-0 w-screen h-screen border-b bg-white z-50 flex flex-col gap-8 p-5 px-8 md:hidden">
    <div class="flex items-center justify-between gap-4 border-b pb-2">
      ...
      <button on:click={() => ($openModal = false)} class="outline-none border-none"><i class="fa-solid fa-xmark text-2xl"></i></button>
    </div>

    <div class="flex flex-col gap-12 flex-1 items-center justify-center">
      <button  on:click={() => reroute("#products")} 
    ...
    <div class="flex flex-col items-center justify-center"><CTAs /></div>
  </div>
{/if}
